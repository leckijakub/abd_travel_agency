...
def test_start():
    ...
    db.session.add_all([
        ...
        Client(uid="4", email="a3@a", password=generate_password_hash("a", method='sha256'),name="John",surname="Fucking Wick", address="Warszawa",phone_number="133 742 069"),
    ...
    db.session.commit()
    db.session.add(Reservation(price=444, status='test offer', client_id=User.query.first().id, employee_id = None, offer_id = offer1.id))
    db.session.commit()
